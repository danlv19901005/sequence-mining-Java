...
setwd("C:/Users/Dan Lv/OneDrive/Summer Intern/OneDrive/SimStudent Project/Studies (where we collect data)/Study VI/Data/Master Wide Data")
...
data1=data.CAIPP = subset(data, pre_class=="bad")
data1=data.CAIPP = subset(data1, post_class=="medium")


plot(data1$Pre_PST_PercentCorrect,data1$normalized.gain)
summary(data1$normalized.gain)
...
data1 = read.csv("C:/Users/Dan Lv/OneDrive/Summer Intern/OneDrive/SimStudent Project/Studies (where we collect data)/Study VI/Data/Master Wide Data/MasterWide_StudyVI_TestScores_V5.csv")
dim(data1)
data1$Pre_PST_PercentCorrect <- as.numeric(as.character(data$Pre_PST_PercentCorrect))
#data1$Post_PST_PercentCorrect <- as.numeric(as.character(data$Post_PST_PercentCorrect))
data1$Total.QXT<-sapply(data1$Total.QXT, as.numeric)

data.CAIPP = subset(data1, CAIPP==1)
dim(data.CAIPP)

data.DSPM = subset(data.CAIPP, hasDSPM==1)
dim(data.DSPM)
b<-lm(data.DSPM$Post_PST_PercentCorrect~data.DSPM$Pre_PST_PercentCorrect+data.DSPM$Total.QXT)
anova(b)
summary(b)
...
# Reading data
#
data2 = read.csv("C:/Users/Dan Lv/Documents/TXQ_Frequency_regression_g2.csv")
dim(data2)
# [1] 376 129

lm2 <- lm(data2$post_test~data2$pre_test+data2$QXT+data2$QTX+data2$TQX+data2$TXQ+data2$XQT+data2$XTQ)
anova(lm2)
summary(lm2)
require(MASS)
...
lm2 <- lm(data2$post_test~data2$pre_test+data2$condition+data2$QXT+data2$QTX+data2$XQT)
anova(lm2)
summary(lm2)



a<-lm(data2$post_test~data2$pre_test+data2$Total_XTQ)

#summary(data2$pre_test)
#summary(data.DSPM$Total.QXT)
...
lm3 <- lm(data2$post_test~data2$pre_test+data2$QXT+data2$XQT+data2$XTQ)
anova(lm3)
summary(lm3)


lm3 <- lm(data2$post_test~data2$pre_test+data2$QXT)
anova(lm3)
summary(lm3)
